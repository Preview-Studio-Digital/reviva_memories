 ...
 * Adapted to Vanilla JS from the original Three.js React component.
 */
window.initLiquidEther = function(container, options = {}) {
  ...
  function makePaletteTexture(stops) {
    ...
    // Generate a high-resolution 256-pixel wide smooth gradient
    const w = 256;
    const data = new Uint8Array(w * 4);
    const colorObjects = arr.map(hex => new THREE.Color(hex));
    
    for (let i = 0; i < w; i++) {
      const pct = i / (w - 1);
      const segment = pct * (colorObjects.length - 1);
      const idx = Math.floor(segment);
      const nextIdx = Math.min(idx + 1, colorObjects.length - 1);
      const factor = segment - idx;
      
      const c1 = colorObjects[idx];
      const c2 = colorObjects[nextIdx];
  ...
  class CommonClass {
    ...
    init(container) {
      ...
      // Explicitly query WebGL extensions to enable float linear filtering
...
`;
  ...
  class Simulation {
    ...
    getFloatType() {
      // Return HalfFloatType for wider linear-filtering GPU support
